...
//var textureBack = texloader.load('res/back.jpg');
var textureTop = texloader.load('res/top.jpg');
var mat = new THREE.MeshBasicMaterial({transparent: true, opacity: 0.7, color: 0xde0013})
var topMat = new THREE.MeshBasicMaterial({transparent: true, opacity: 1.0	, map: textureTop})
var mats1 = []
mats1.push(mat);
mats1.push(mat);
mats1.push(mat);
...
Game.prototype = {
  init: function() {
    ...
    this._setLight() // 设置光照
    this._createCube(mats1) // 加一个方块
    this._createCube(mats1) // 再加一个方块
  ...
  _createJumper: function() {
    ...
    this.human = new r.Object3D, this.head = new r.Mesh(new r.SphereGeometry(1 * .45, 20, 20), t), this.head.castShadow = !0, this.bottom = new r.Mesh(new r.CylinderGeometry(.4158, .60007, 1 * .45 * 2.68, 20), new r.MeshBasicMaterial({
      //map: texloader.load("res/body.png")
      color: 0x990000
  ...
  _setLight: function() {
    ...
    var directionalLight = new THREE.DirectionalLight(0xcccccc, .28);
  ...
  _setRenderer: function() {
	...
	this.renderer.setClearAlpha(this.config.backAlpha)
    ...
    //添加平面, 绘制阴影
    //var planeGeometry = new THREE.PlaneGeometry(this.size.width, this.size.height, 1, 1);
    //var texloader = new THREE.TextureLoader();
